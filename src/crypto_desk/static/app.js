...
function render(r) {
  ...
  const executionHtml = execution ? `
    <div class="section"><h3>3. 戰術執行面板</h3>
      ...
      <div class="cards">
        <div class="card"><span>ENTRY ZONE</span><b>${execution.entry_zone}</b><small>${execution.entry_style}</small></div>
        <div class="card"><span>HARD SL</span><b class="negative">${execution.stop}</b><small>${execution.stop_reason}</small></div>
        <div class="card"><span>R:R</span><b>${execution.rr}:1</b><small>${execution.rr < 2.5 ? "低於門檻，否決" : "通過 1:2.5 門檻"}</small></div>
        <div class="card"><span>TP1</span><b>${execution.tp1}</b><small>防守型止盈</small></div>
        <div class="card"><span>TP2</span><b>${execution.tp2}</b><small>主要流動性目標</small></div>
        <div class="card"><span>TP3</span><b>${execution.tp3}</b><small>移動止損管理</small></div>
      </div>
      <p>ATR：${execution.atr}${execution.atr_inferred ? "（缺值，以價格 2% 情境推估）" : ""}</p>
    </div>
    <div class="section"><h3>部位與風險</h3>
      <div class="cards">
        <div class="card"><span>風險預算</span><b>${risk.risk_budget_pct}%</b><small>1/4 Kelly，硬上限 1%</small></div>
        <div class="card"><span>名目曝險</span><b>${risk.exposure_pct}%</b><small>${risk.classification}</small></div>
        <div class="card"><span>95% VaR 情境</span><b>$${money(risk.var95)}</b><small>Beta ${risk.beta_used} / ATR 模型</small></div>
      </div>
    </div>` : "";
  reportNode.classList.remove("empty");
  reportNode.innerHTML = `
    <div class="memo-head"><div><div class="eyebrow">📊 機構級交易決策備忘錄</div>
      <h2>${input.asset} · ${input.timeframe} · ${input.direction.toUpperCase()}</h2>
...
function updateLive(report) {
  ...
  document.querySelector("#live-change").textContent = `${market.change24h >= 0 ? "+" : ""}${market.change24h.toFixed(2)}% / 24H`;
  document.querySelector("#live-bias").textContent = report.input.direction.toUpperCase();
  document.querySelector("#live-bias").className = report.input.direction === "long" ? "positive" : "negative";
  document.querySelector("#live-score").textContent = `confluence ${report.score}`;
  ...
  document.querySelector("#live-regime").textContent = market.adx >= 25 ? "TREND" : "RANGE / TRANSITION";
  document.querySelector("#live-atr").textContent = money(market.atr);
  document.querySelector("#live-bb").textContent = `BB width ${market.bbWidthPct.toFixed(2)}%`;
}
...
form.addEventListener("submit", async event => {
  ...
  try {
    ...
    button.textContent = "RUN INSTITUTIONAL ANALYSIS";
  }
});
